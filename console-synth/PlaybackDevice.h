...
public:
	...
	/// </summary>
	virtual int WritePlaybackBuffer(void* playbackBuffer, unsigned int numberOfFrames, double streamTime, bool& hasOutput) = 0;

	/// <summary>
	/// Tells the playback device to write its output to the playback buffer. The write should start at frame index 0, and
	/// end at the frame index that coincides with the stream end time. The function should return non-zero for error indication.
	/// </summary>
	virtual int WritePlaybackBuffer(PlaybackBuffer<TSignal>* playbackBuffer, unsigned int numberOfFrames, double streamTime, bool& hasOutput) = 0;
};
